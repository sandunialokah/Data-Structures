...
class Graph():
    ...
    def pop(self, pqueue, MST, v):
        for i in range(pqueue.qsize()):
            if pqueue.queue[i][1] == v:
                for j in range(self.size):
                    if MST.seek_matrix(pqueue.queue[i][2], j) < float('inf'):
                        pqueue.queue.remove((pqueue.queue[i][0], v, pqueue.queue[i][2]))
    ...
    def PMST(self, s):
        ...
        while not pqueue.empty() and e != n:
            ...
            if visited[min_edge[2]]:
                ...
                # added to remove redundant entires in queue to make it eager
                self.pop(pqueue, MST, min_edge[2])
        
        print()
        print("Prim's Minimum Spanning Tree(Eager): ")
        MST.print_matrix()
        print()
